        ...
        if individual_feedback:
            ...
            for remark in remarks:
                if remark.get('id'):
                    ...
                    role_remark = instance.role.remarks.get(
                        id=remark.get('id')
                    )
                    ...
                    rating = Rating.objects.get(
                        pk=remark.pop('rating').get('rating_id')
                    )
    ...
    def validate(self, attrs):
        ...
        if self.instance:
            if self.instance.round:
                # Round isn't a required field, so only check for end date if
                # there's a round.
                feedback_closed =
                datetime.now(timezone.utc) > self.instance.round.end_date
            else:
                # Otherwise we expire editing of feedback after 7 days.
                feedback_closed =
                self.instance.date +
                timedelta(days=7) < datetime.now(timezone.utc)
        ...
        if not self.partial:
            ...
            if (individual_feedback and role_feedback)
            or (not individual_feedback and not role_feedback):
